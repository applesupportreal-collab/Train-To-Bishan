...
- `trainBreakdown.chance` controls the odds that the train breaks down on each station-to-station travel leg.
- `trainBreakdown.minSteps` and `trainBreakdown.maxSteps` set the random walking target after a breakdown.
- `trainBreakdown.motionThreshold`, `trainBreakdown.motionResetThreshold`, and `trainBreakdown.minStepInterval` tune the approximate step detector.
...
- `seatRush`, `seatOffer`, `trainBreakdown`, `trainDelay`, `upright`, `trainSound`, `announcement`, `auntieEvent`, and `vibration` control the other game mechanics.
